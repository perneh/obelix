...
from tests.support.builders.api import build_encode_request, build_template_payload
...
@pytest.mark.integration
def test_save_and_load_template(api_client):
    ...
    saved = run_save_template(api_client, payload)
    assert_status_ok(saved)

    loaded = run_load_template(api_client, "test-template")
    assert_status_ok(loaded)
    assert_json_field(loaded, "name", "Test")
